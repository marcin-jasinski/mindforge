...
export class AuthService {
  ...
  loadCurrentUser() {
    return this.api.get<UserResponse>('/api/me').pipe(
  ...
  register(req: RegisterRequest) {
    return this.api.post<UserResponse>('/api/register', req).pipe(
      tap(user => this._user.set(user)),
    );
  }

  login(req: LoginRequest) {
    return this.api.post<TokenResponse>('/api/login', req).pipe(
  ...
  logout() {
    return this.api.post<void>('/api/logout', {}).pipe(
      tap(() => {
        this._user.set(null);
